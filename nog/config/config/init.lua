...
function event_handlers(event_name)
  return function(tbl)
    nog.on(event_name, function(ev)
      for _, handler in ipairs(tbl) do
...
event_handlers "win_manage" {
  {
    when = function(ev)
      ...
      return (size.width <= 100 or size.height <= 100)
        or (title:find("^League of Legends$"))
    end,
    action = function(ev)
  ...
  },
  {
    when = function(ev)
      local title = nog.win_get_title(ev.win_id)
      return title:find("^.* - Mozilla Firefox$")
    end,
    action = function(ev)
      if ev.ws_id ~= 2 then
...
event_handlers "ws_created" {
  {
    when = function(ev)
      return ev.ws_id == 1
    end,
    action = function(ev)
      nog.ws_set_name(ev.ws_id, "Code")
    end
  }
}
